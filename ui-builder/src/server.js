...
import { DesignSystemMCP } from './services/DesignSystemMCP.js';
...
const uiBuilderAgent = new UIBuilderAgent(designSystemMCP);
...
app.get('/api/components', async (req, res) => {
  try {
    const components = await designSystemMCP.listComponents();
    res.json(components);
  } catch (error) {
...
app.get('/api/components/:name', async (req, res) => {
  try {
    ...
    const component = await designSystemMCP.getComponent(name);
    res.json(component);
  } catch (error) {
