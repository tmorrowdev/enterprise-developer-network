...
class DesignSystemServer {
  ...
  private async loadDesignSystem() {
    await this.loadCre8ComponentsFromFile();
    await this.loadDesignTokens();
  }

  private async loadCre8ComponentsFromFile() {
    try {
      const componentsFilePath = path.join(process.cwd(), 'cre8-wc-components.md');
      if (!fs.existsSync(componentsFilePath)) {
        console.warn('cre8-wc-components.md not found, loading default components');
        this.loadCre8Components();
        return;
      }

      const content = fs.readFileSync(componentsFilePath, 'utf-8');
      const components = this.parseComponentsFromMarkdown(content);
      
      components.forEach(component => {
        this.components.set(component.name.toLowerCase(), component);
      });

      console.log(`Loaded ${components.length} components from cre8-wc-components.md`);
    } catch (error) {
      console.error('Error loading components from file:', error);
      this.loadCre8Components(); // Fallback to default components
    }
  }

  private parseComponentsFromMarkdown(content: string): Component[] {
    const components: Component[] = [];
    const sections = content.split(/^## /m).slice(1); // Split by ## headers, skip first empty section
    const seenComponents = new Set<string>(); // Track duplicates

    for (const section of sections) {
      const lines = section.split('\n');
      const componentName = lines[0].trim();
      
      // Skip if this is not a valid component section
      if (!componentName || 
          componentName.includes('Table of Contents') ||
          componentName.includes('Appendix') ||
          componentName.includes('ACCESSIBILITY NOTE') ||
          componentName.includes('How to use') ||
          componentName.includes('Usability Guidelines') ||
          componentName.includes('Determinable vs indeterminate progress') ||
          componentName.includes('Alert Styles:') ||
          !this.isValidComponentName(componentName)) {
        continue;
      }

      // Skip duplicates
      const normalizedName = componentName.toLowerCase();
      if (seenComponents.has(normalizedName)) {
        continue;
      }
      seenComponents.add(normalizedName);

      // Convert component name to kebab-case tag name
      const tagName = this.convertToKebabCase(componentName);
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (line.startsWith('### ') && line !== '### Overview') {
          inOverview = false;
        }
        
        if (line.startsWith('### File Location')) {
          // Next line should contain the file path
          if (i + 1 < lines.length) {
            fileLocation = lines[i + 1].trim().replace(/`/g, '');
          }
          break;
        }
      ...
      const component: Component = {
        name: componentName,
        tagName: tagName,
        description: description || `${componentName} component for the Cre8 design system.`,
        props: [], // We'll add basic props since detailed parsing would be complex
        examples: [{
          title: `Basic ${componentName}`,
          description: `Simple ${componentName} usage`,
          code: `<${tagName}></${tagName}>`
        }],
        usage_frequency: 50 // Default frequency
      };

      // Add some common props based on component type
      this.addCommonProps(component);
  ...
  private isValidComponentName(name: string): boolean {
    // Check if it's a valid component name (should be alphanumeric with possible hyphens)
  ...
  private convertToKebabCase(componentName: string): string {
    // Remove 'Cre8' prefix if present
    let name = componentName.replace(/^Cre8/, '');
    
    // If the name is already kebab-case, just add the prefix
    if (name.includes('-')) {
      return `cre8-${name}`;
    }
    ...
    // Remove leading dash and add cre8- prefix
    name = name.replace(/^-/, '');
    
    return `cre8-${name}`;
  }

  private addCommonProps(component: Component) {
    // Add common props based on component name/type
    const commonProps: ComponentProperty[] = [];
    
    // All components get these basic props
    commonProps.push({
      name: 'id',
      type: 'string',
    ...
    if (name.includes('button')) {
      commonProps.push(
        { name: 'variant', type: 'string', description: 'Button style variant', options: ['primary', 'secondary', 'tertiary'], default: 'primary' },
        ...
        { name: 'text', type: 'string', description: 'Button text content' }
      );
    }
    
    if (name.includes('field') || name.includes('input')) {
      commonProps.push(
...
<cre8-button text="Large" size="lg"></cre8-button>`
      ...
      tools: [
        ...
        },
        {
          ...
          inputSchema: {
            type: 'object',
            properties: {
              format: { type: 'string', enum: ['detailed', 'names-only'], description: 'Output format - detailed list or just component names', default: 'detailed' }
            }
          }
        },
        {
  ...
  private async listComponents(args: any) {
    ...
    if (format === 'names-only') {
      const componentNames = components.map(comp => comp.tagName).join('\n');
      return {
        content: [{
          type: 'text',
          text: `# Component Tag Names\n\n${componentNames}`
        }]
      };
    }
  ...
  private async getDesignTokens(args: any) {
    ...
    Object.entries(tokensByCategory).forEach(([cat, tokenList]) => {
      ...
      Array(tokenList).forEach((token: any) => {
